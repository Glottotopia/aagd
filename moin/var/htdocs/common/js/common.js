...
//     alert(1)
    IDS = []
    $( ".idselector" ).each(function(i,table){  
	span = $(table).find( ".anchor" ); 
	ID =  span.attr('id'); 
	IDS.push(ID)
// 	$( this ).after('<div class="queryinfo">Content fetched from <a href="'+querystring.replace(/"/g,'%22')+'" target="_blank">'+querystring.replace('&','&amp;')+'</a></div>'); 
	$(table).after('<div  class="results" id="qscr'+ID+'"></div>'); 

    })  
    for(i=0;i<IDS.length;i++){
	ID = IDS[i] 
	querystring = 'http://www.glottotopia.org/solr/aagd/query?q=id%3A'+ID+'&fl=id,lingex';
	$.getJSON(querystring, function(data) {  
	    doc=data.response.docs[0]
	    IDv = doc.id
	    $('#qscr'+IDv).append('<div class="result-document"><div>'+IDv+'</div><div class="lingex">'+doc.lingex.replace(/\[/g,'<').replace(/\]/g,'>')+"</div></div>")  
	});  
    }	  
    
    q = $( ".exemplarselector td p:first" ); 
    querystring = 'http://www.glottotopia.org/solr/aagd/query?q='+q.text().trim()+'&fl=lingex,id';
    $('<div id="querystringcontainer" class="qsc">Content fetched from <a href="'+querystring.replace(/"/g,'%22')+'" target="_blank">'+querystring.replace('&','&amp;')+'</a></div>').insertAfter('.exemplarselector:first'); 
    $('<div id="exemplarcontainer" class="results"></div>').insertAfter('#querystringcontainer'); 
    
    $.getJSON(querystring, function(data) {
	docs = data.response.docs;
        for (i=0;i<docs.length;i++){ 
	    $('#exemplarcontainer').append('<div class="result-document"><div>'+docs[i].id+'</div><div class="lingex">'+docs[i].lingex.replace(/\[/g,'<').replace(/\]/g,'>')+"</div></div>")  
        }
    });
